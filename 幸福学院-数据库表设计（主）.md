...
2. 添加字段：  在商品详情表中添加销量（product_sales）字段以及实付款（actual_payment）                                    
3. 修改字段：   商品一级分类表，二级分类表 ，商品详情表中的 frist修改为first； 订单表详情表中将优惠卷抵扣（coupon）字段修改为优惠券抵扣ID、买家地址（address）字段修改为买家地址ID、优惠券抵扣（coupon）字段修改为优惠券抵扣ID；
