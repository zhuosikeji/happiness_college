...
| v1-1.2     | 2019.01.01   | 侯宇飞     | AB           | 已修订       | 2019.01.01   |            |            |
| v1-1.3     | 2019.01.02   | 侯宇飞     | A            | 已修订       | 2019.01.02   |            |            |
...
2. 添加字段：  在商品详情表中添加销量（product_sales）字段以及实付款（actual_payment）；退货表中添加积分记录字段（hc_refund)，添加退货理由（apply_reason）；   用户表地址表中添加字段收货人名称(user_name)、收货人手机（user_phone）、区域id(area_Id_path) 、是否删除（is_delete）                      
...
| is_delete           | 是否删除       | int           | 2        | N              | Y            | 1.删除  0.保留                                               |
| roduct_uppershelf   | 是否上架       | int           | 2        | N              | Y            | 1-上架   0-下架                                              |
