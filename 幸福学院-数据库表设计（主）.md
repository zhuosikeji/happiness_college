...
2. 添加字段：  在商品详情表中添加销量（product_sales）字段以及实付款（actual_payment）；退货表中添加积分记录字段（hc_refund)）；   用户表地址表中添加字段收货人名称(user_name)、收货人手机（user_pnone）、区域id(user_address)                           
...
| is_default   | 有效状态     | int      | 4        | N              | Y            | 1：有效 -1：无效 |
