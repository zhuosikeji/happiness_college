...
2. 添加字段：  在商品详情表中添加销量（product_sales）字段以及实付款（actual_payment）；退货表中添加积分记录字段（hc_refund)）；   用户表地址表中添加字段收货人名称、收货人手机、区域id路径                           
...
| ID               | 编号         | varchar  | 36       | **Y**          | **N**        |                |
| user_id          | 买家ID       | varchar  | 36       | N              | Y            |                |
| user_name        | 收货人       | varchar  | 36       | N              | Y            |                |
| user_phone       | 收货人手机号 | varchar  | 36       | N              | Y            |                |
| areaId_path      | 区域id路径   | varchar  | 255      | N              | Y            | 省Id_市Id_县Id |
| detailed_address | 详细地址     | varchar  | 255      | N              | Y            |                |
